...
class PySysTest(AnalyticsBaseTest):
	def execute(self):
		...
		self.injectASAI(correlator)
		self.ready(correlator)

		correlator.send('Config.evt')
		self.waitForSignal('correlator.out',
						   expr='Analytic ASAI started for inputDataNames',
						   condition='==1',
						   timeout=5)
		
	def validate(self):
		self.assertGrep('correlator.out', expr='Mandatory param totalCustomers missing', condition='==3')
		self.assertGrep('correlator.out', expr='Mandatory param timePeriod missing', condition='==4')
		...
		self.assertGrep('correlator.out', expr='Param timePeriod must be greater than zero', condition='==1')
		self.assertGrep('correlator.out', expr='Error spawning ASAI instance', condition='==8')
		self.checkSanity()	

...
class PySysTest(AnalyticsBaseTest):
	def execute(self):
		...
		correlator.receive(filename='Output22.evt', channels=['Output22'])
		correlator.receive(filename='Output23.evt', channels=['Output23'])
		correlator.receive(filename='Output24.evt', channels=['Output24'])
		correlator.receive(filename='Output25.evt', channels=['Output25'])
		correlator.receive(filename='Output26.evt', channels=['Output26'])
	...
	def validate(self):
		...
		self.assertDiff('Output22.evt', 'Output22.evt')
		self.assertDiff('Output23.evt', 'Output23.evt')
		self.assertDiff('Output24.evt', 'Output24.evt')
		self.assertDiff('Output25.evt', 'Output25.evt')
		self.assertDiff('Output26.evt', 'Output26.evt')
		
		# Make sure that the we got the right number of passes/fails
		self.assertLineCount('correlator.out', expr='Validating com.industry.analytics.Analytic\("Expression",.*\)', condition='==26')
		self.assertLineCount('correlator.out', expr='Error spawning Expression instance.', condition='==0')
		self.assertLineCount('correlator.out', expr='Analytic Expression started for inputDataNames', condition='==26')

...
class PySysTest(AnalyticsBaseTest):
	def execute(self):
		...
		correlator.receive(filename='Param Offset.evt', channels=['Param Offset'])
		correlator.receive(filename='Cartesian Fixed Offset.evt', channels=['Cartesian Fixed Offset'])
		correlator.receive(filename='Cartesian Param Offset.evt', channels=['Cartesian Param Offset'])
		correlator.receive(filename='Fixed Radius.evt', channels=['Fixed Radius'])
		correlator.receive(filename='Param Radius.evt', channels=['Param Radius'])
		correlator.receive(filename='Cartesian Fixed Radius.evt', channels=['Cartesian Fixed Radius'])
		correlator.receive(filename='Cartesian Param Radius.evt', channels=['Cartesian Param Radius'])
		...
		correlator.receive(filename='Param Polygon.evt', channels=['Param Polygon'])
		...
		correlator.receive(filename='Cartesian Param Polygon.evt', channels=['Cartesian Param Polygon'])
		self.ready(correlator)

		correlator.send('Config.evt')
		self.waitForSignal('correlator.log', expr='Analytic GeoFence started for inputDataNames', condition='==14', timeout=5)
		correlator.send('Events.evt')
		self.waitForSignal('Cartesian Param Polygon.evt', expr='com\.industry\.analytics\.Data\(', condition='==15', timeout=5)

		
	def validate(self):
		self.assertDiff('Fixed Offset.evt', 'Fixed Offset.evt')
		self.assertDiff('Param Offset.evt', 'Param Offset.evt')
		self.assertDiff('Cartesian Fixed Offset.evt', 'Cartesian Fixed Offset.evt')
		self.assertDiff('Cartesian Param Offset.evt', 'Cartesian Param Offset.evt')
		self.assertDiff('Fixed Radius.evt', 'Fixed Radius.evt')
		self.assertDiff('Param Radius.evt', 'Param Radius.evt')
		self.assertDiff('Cartesian Fixed Radius.evt', 'Cartesian Fixed Radius.evt')
		self.assertDiff('Cartesian Param Radius.evt', 'Cartesian Param Radius.evt')
		self.assertDiff('Fixed Polygon.evt', 'Fixed Polygon.evt')
		self.assertDiff('Relative Fixed Polygon.evt', 'Relative Fixed Polygon.evt')
		self.assertDiff('Param Polygon.evt', 'Param Polygon.evt')
		...
		self.assertDiff('Cartesian Param Polygon.evt', 'Cartesian Param Polygon.evt')
